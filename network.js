function setupNetworkInterface() {
  ...
  const netmapUpdateCb = (map) => {
    ...
    const rmb_to_copy = (event) => {
      ...
      window.navigator.clipboard.writeText(ip).catch((msg) => {
        console.log("network.js: Copy ip to clipboard: Error: " + msg);
      });
...
function registerNetworkLogin(
  cx,
  { authKey, statusElem, loginElem, loginElemUrl, loginPromise },
) {
  if (authKey) {
    if (loginElemUrl) {
      loginElem.href = loginElemUrl;
